...
router.get("/", function(req, res) {
  db.getAllArticles(action = (rows) => { //get articles... when gotten...
    ...
    rows.forEach(row => { //divide rows into a list of articles
      ...
      row.content = row.content.substring(0,500);
    ...
    db.getUser("mfigurski80", action = (profile) => {
...
router.get("/articles/:article", function(req, res, next) {
  db.getArticle(title = req.params.article, action = (article) => {
    res.render('article', {
      article: article
    });
  });
...
router.post("/articles/:article", function(req, res) {
  if (isReqAuthorized(req)) {
    ...
    db.addArticle(req.body.title, req.body.content, req.body.type, req.body.imageFile);
...
router.put("/articles/:article", function(req, res) {
  if (isReqAuthorized(req)) {
    db.updateArticle(req.params.article, req.body.title, req.body.content);
    res.send("completed");
  } else {
...
router.post("/login", function(req, res) {
  ...
  db.getUser(req.body.username, action = (user) => {
    if (curSessions[id] && SHA256(req.body.password + user.salt) == user.password) {
      curSessions[id].user.isLoggedIn = true;
      curSessions[id].user.username = req.body.username;
      curSessions[id].user.name = user.name;
...
router.get("/admin", function(req, res) {
  if (isReqAuthorized(req)) {
    db.getUser(curSessions[req.cookies.SessionID].user.username, action=function(user) {
      res.render('admin', {
        user: user
      });
    });
